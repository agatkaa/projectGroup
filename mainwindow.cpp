...
// funkcja dodajaca wiersz
void MainWindow::on_addRowButton_pressed()
{

    // zwiekszenie zmiennej przechowujacej ilosc wierszy
    int row = tablice.at(iteratorTablic)->table.rowNumber++;

    // dodanie tylu labeli ile jest kolumn w ostatnim wierszu
...
// przelaczanie pomiedzy tablicami
void MainWindow::on_move_right_button_pressed()
{
    tablice.at(iteratorTablic)->hide(); //ukrywamy obecna
    iteratorTablic=iteratorTablic+1; // przesuwamy iterator wskazujacy na tablice
    update();
...
void MainWindow::on_dodajTablice_pressed(){
    tablica = new Tablica (this, &repository); //tworzenie nowej tablicy
    tablice.push_back(tablica);
    tablice.at(iteratorTablic)->hide(); // ukrycie poprzedniej widocznej tablicy
    iteratorTablic = tablice.size()-1; //ustawienie iteratora na najnowsza tablice
    update();
}
...
{
    // zabezpieczenie, ze jezeli jestesmy na pierwszej tablicy to juz nie mozemy przewijac w lewo
    // a na ostatniej w prawo
    if (iteratorTablic==0) {
...
// funkcja pomocnicza do wyboru pliku, mozliwe wybranie tylko odpowiednich formatow plikow przy wczytywaniu obrazkow
